...
public class NoteRepository {
  ...
  private Note create(Note note) {
    String sql = """
      INSERT INTO tasknote.notes (
        user_id, description, title, last_update, shared, share_token, archived)
      VALUES (:userId, :description, :title, :lastUpdate, :shared, :shareToken, :archived)
  ...
  public Optional<Note> findById(Long id) {
    ...
    MapSqlParameterSource params = new MapSqlParameterSource()
        .addValue("id", id);
    List<Note> notes = jdbcTemplate.query(sql, params, new NoteRowMapper());
  ...
  public List<Note> findAllBySearchTerm(Long userId, String searchTerm) {
    String sql = """
      ...
      FROM tasknote.notes
      WHERE user_id = :userId
        AND (upper(title) like upper(concat('%', :searchTerm, '%')) OR upper(description) like upper(concat('%', :searchTerm, '%')))
      """;
    MapSqlParameterSource params = new MapSqlParameterSource()
        .addValue("userId", userId)
